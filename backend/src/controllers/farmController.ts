...
// Delete an activity from a plot
export const deletePlotActivity = async (req: Request, res: Response) => {
    try {
        const { farmId, plotNumber, activityId } = req.params;
        const userId = req.user!._id;
        ...
        const farm = await Farm.findOne({
            _id: farmId,
            owner: userId,
            // isActive: true, // Allow deleting from inactive farms? Probably strictly mostly active.
        });
        ...
        const activityIndex = farm.plots[plotIndex].activities.findIndex(
            (activity: any) => activity._id.toString() === activityId,
        );

        if (activityIndex === -1) {
            logger.warn(
                `Activity ${activityId} not found in plot ${plotNumber}. Available IDs: ${farm.plots[
                    plotIndex
                ].activities.map((a: any) => a._id.toString())}`,
            );
            return res.status(404).json({
                success: false,
        ...
        farm.plots[plotIndex].activities.splice(activityIndex, 1);
        await farm.save();

        logger.info(
            `Activity ${activityId} deleted from plot ${plotNumber} in farm ${farmId} by user ${userId}`,
        );
        ...
        logger.error("Delete plot activity error:", error);
        res.status(500).json({
            success: false,
...
export const getDashboardData = async (req: Request, res: Response) => {
    try {
        ...
        // Save farms to persist any newly generated subdocument IDs (lazy migration)
        await Promise.all(farms.map((f) => f.save()));
